...
(Note: The ~itemLabel~ was later added to the query but Wikidata
struggled to deliver the complete results. Hence, when less results
are returned, try removing the ~itemLabel~ from the ~SELECT~
statement.)
...
memory and thus requires an enormous amount of main memory.
