...
fictional literary works) in the [[http://www.dnb.de/][German National Library]]
(DNB). Therefore, we
...
It's not trivial to extract all novels from a big catalogue like that
...
al. 2013](https://arxiv.org/abs/1309.3323)) and we encounter the same
problem which calls for an innovative solution.
...
The result is in [[file:wikidata_writer_subclass.tsv][wikidata_writer_subclass.tsv]].
...
We can see the reason for this problem by checking the end of
[[file:wikidata_writer_subclass.tsv][wikidata_writer_subclass.tsv]]:
...
Not all writers are returned due to a query timeout which is likely be
caused by the expensive [[https://www.wikidata.org/wiki/Wikidata:SPARQL_query_service/queries#Querying_a_class_tree][path query]].
...
** downloading the different parts and joining them
...
- all entities with an "occupation" (P106) property
- all subclasses of "writer" (Q36180)
- all entities with a "GND id" (P227) property

We can then join the resulting three files and get a list of writers
together with their GND id.

*** entity has an occupation property
...
We download all entities which have a GND id:
...
#+BEGIN_SRC sh :results silent
  curl \
      ...
      --globoff \
       'https://query.wikidata.org/sparql?query=%20%20SELECT%20%3Fitem%20%3Fgndid%0A%20%20WHERE%0A%20%20{%0A%20%20%20%20%3Fitem%20p%3AP227%20%3Fwds%20.%20%0A%20%20%20%20%3Fwds%20%3Fv%20%3Fgndid%20.%0A%20%20%20%20%3FwdP%20wikibase%3Aclaim%20p%3AP227%20.%0A%20%20%20%20%3FwdP%20wikibase%3AstatementProperty%20%3Fv%20.%0A%20%20%20%20SERVICE%20wikibase%3Alabel%20{%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%23%20...%20include%20the%20labels%0A%20%20%20%20%20%20bd%3AserviceParam%20wikibase%3Alanguage%20%22en%22%0A%20%20%20%20}%0A%20%20}'
#+END_SRC
...
Due to [[https://unix.stackexchange.com/questions/12942/join-file-2-not-in-sorted-order][an incompatibility]] between my ~sort~ and ~join~ program, we
...
It is slow but it works pretty well. 

*** joining writers and books

We extend the Python code to check for each GND whether it appears or
list from Wikidata (~wd_results~):
...
Just out of curiosity: what are the [[https://www.wikidata.org/wiki/Property:P279][subclasses of (P279)]]
[[https://www.wikidata.org/wiki/Q36180][writer (Q36180)]] ([[https://query.wikidata.org/#%2520%2520SELECT%2520%253Fs%2520%253Fdesc%250A%2520%2520WHERE%250A%2520%2520{%250A%2520%2520%2520%2520%253Fs%2520wdt%253AP279%2520wd%253AQ36180%2520.%250A%2520%2520%2520%2520OPTIONAL%2520{%250A%2520%2520%2520%2520%2520%2520%253Fs%2520rdfs%253Alabel%2520%253Fdesc%2520filter%2520%2528lang%2528%253Fdesc%2529%2520%253D%2520%2522en%2522%2529.%250A%2520%2520%2520%2520}%250A%2520%2520}%250A%2520%2520ORDER%2520BY%2520%253Fdesc%250A][SPARQL]]):
