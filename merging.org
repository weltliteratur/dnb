...
#+BEGIN_SRC sh
  LC_ALL=C \
	./json2json.py -p "issued,pages_norm,creator_wd.*.occupation_writer" DNBTitel_normalised_enriched.json.gz \
      | awk -F'\t' '{if ($1 != "" && $2 != "" && $3 != "") print int($1/10)"\t"$2}' \
      | sort -S1G -nr \
      | datamash -g 1 median 2 mean 2 min 2 max 2 count 2\
...
plot \
  'decade_pages_stats.tsv' using ($1*10):2 with linespoints pt 7 title 'median',\
  'decade_pages_stats.tsv' using ($1*10):3 with linespoints pt 7 title 'mean'
